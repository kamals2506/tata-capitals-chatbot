        ...
        if ($nextStepId <= 0) {
            ...
            */
            ...
            if ($stepKey === 'tenure') {
                ...
                // Return to Home Loan information menu.
                $homeLoanInfoStep = $db->table('chatbot_steps')
                    ->where('step_key', 'home_loan_info')
                    ->where('status', 'active')
                    ->orderBy('id', 'ASC')
                    ->get()
                ...
                if ($homeLoanInfoStep) {
                    ...
                    $this->setCurrentChatbotStep(
                        $sessionId,
                        (int) $homeLoanInfoStep['id']
                    );

                    $homeLoanOptions = $db->table('chatbot_options')
                        ->where(
                            'step_id',
                            (int) $homeLoanInfoStep['id']
                        )
                    ...
                    foreach ($homeLoanOptions as $option) {
                        $formattedOptions[] = [
                            'id'           => (int) $option['id'],
                            'label'        => $option['label'],
                            'value'        => $option['value'],
                    ...
                    $message = $selectedTenure !== ''
                        ? "Okay, great! We've received your enquiry for a Home Loan with a {$selectedTenure} tenure and recorded it for you. What would you like to know next?"
                        : "Okay, great! We've received your Home Loan enquiry and recorded it for you. What would you like to know next?";

                    return [
                        'message'    => $message,
                        'flow_id'    => (int) $homeLoanInfoStep['flow_id'],
                        'step_id'    => (int) $homeLoanInfoStep['id'],
                        'step_key'   => $homeLoanInfoStep['step_key'],
                        'input_type' => $homeLoanInfoStep['input_type'],
