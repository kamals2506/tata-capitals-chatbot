    ...
    private function handleHomeLoanApplicationFlow(int $sessionId, string $userText, string $language = 'en'): ?array
    {
        ...
        switch ($state) {
            ...
            case 'awaiting_credit_consent':
                if ($this->isNo($text)) { $this->setApplicationState($sessionId, 'paused'); return $this->applicationResponse($sessionId, 'No problem. Your details have been saved securely. You can continue the eligibility check later.', [], 'credit_check_paused'); }
                if (!$this->isYes($text)) return $this->applicationPrompt($sessionId, 'Please confirm whether you want to continue with the credit-profile check.', $state, [['id'=>0,'label'=>'✅ Yes','value'=>'yes','next_step_id'=>0,'sort_order'=>1],['id'=>0,'label'=>'❌ Not now','value'=>'no','next_step_id'=>0,'sort_order'=>2]]);
                // Do not invent a credit score. Real production integration must call an authorised bureau API.
                $this->setApplicationState($sessionId, 'credit_check_pending');
                return $this->applicationResponse($sessionId, 'Your consent has been recorded. The credit check requires an authorised credit-bureau integration. Once the bureau response is available, we can assess your eligibility.', [], 'credit_check_pending');
        }
        return null;
    }
