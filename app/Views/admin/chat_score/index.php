            ...
            <tbody>
                ...
                <tr>
                   ...
                   <td>
                        ...
                        <?php else: ?>
                            <span class="text-danger me-2">Not Evaluated</span>
                            <a href="<?= site_url('admin/chat-score/evaluate/'.$r->id) ?>" class="btn btn-sm btn-primary evaluateBtn">
                 ...
                 <td>
                    <?php
                    $first = trim($r->agent_first_message_time ?? '');
                    $last  = trim($r->agent_last_message_time ?? '');
...
<div id="evaluateLoader" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15,23,42,0.75); z-index:99999; align-items:center; justify-content:center; flex-direction:column;">
    <div class="spinner-border text-light" style="width:3.5rem; height:3.5rem;" role="status"></div>
    <div class="text-white mt-3" style="font-size:16px; font-weight:500;">
        Evaluating conversation, please wait...
    </div>
</div>
...
$(document).ready(function() {
   ...
   var table = $('#scoreTable').DataTable({
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                res.forEach(function(r) {
                    ...
                    let replyUI = `<button class="btn btn-info btn-sm replyHistory" data-chat="${r.id}">${r.avg_agent_gap || '-'}</button>`;
                    ...
                    table.row.add([
                        ...
                        r.disposition_name || '',
                        actionUI,
                        r.agent_first_message_time || '-',
                        r.agent_last_message_time || '-',
                        replyUI,
                        complianceUI,
                        r.remarks || ''
                    ]);
            ...
            success: function(data) {
                ...
                if (!data || data.length === 0) {
                    ...
                    data.forEach(function(row) {
                        let sender = (row.sender || '').toLowerCase().trim();
                        ...
                        if (sender == "customer") {
                            ...
                            html += `
                                <div class="system-msg-row">
                                    <small><i class="bi bi-robot me-1"></i>${row.message || ''}</small>
                                    <span class="chat-time">${row.created_at || ''}</span>
                                </div>
                            `;
                        }
            ...
            success: function(res) {
                ...
                html += `
                    <div class="alert alert-info">
                        <strong>Average Agent Reply Gap :</strong> ${res.average_text || '-'} &nbsp;|&nbsp;
                        <strong>Agent First Msg :</strong> ${res.agent_first_message_time || '-'} &nbsp;|&nbsp;
                        <strong>Agent Last Msg :</strong> ${res.agent_last_message_time || '-'} &nbsp;|&nbsp;
                        <strong>Chat Duration :</strong> ${res.chat_duration_text || '-'}
                    </div>
                `;
    ...
    $('#evaluateLoader').css('display', 'flex');
});
