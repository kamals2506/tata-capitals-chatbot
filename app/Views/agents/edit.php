...
<style>
    ...
    .hint { font-size:12px; color:#6b7280; margin-top:-12px; margin-bottom:16px; }
</style>
...
<div class="form-card">
    ...
    <form action="<?= site_url('agents/update/' . $agent['id']) ?>" method="post">
        <?= csrf_field() ?>
        ...
        <div class="form-group">
            <label>
                <input type="checkbox" name="is_online" value="1" <?= $agent['is_online'] ? 'checked' : '' ?>>
                Active
            </label>
        </div>

        <a href="<?= site_url('agents') ?>" class="btn-cancel">Cancel</a>
        <button type="submit" class="btn-gradient">Update Agent</button>
    </form>
</div>
