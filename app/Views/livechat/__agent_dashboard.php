...
function loadHistory(chatId)
{
    ...
    $.get(
        ...
        function(res){
            ...
            // Prime the Assist panel with the customer's most recent
            // message from history, so it's ready as soon as the agent
            // opens the chat (does not spam a search per historic line).
        ...
        switch (data.type) {
            ...
            case 'message':
                if (data.chat_id == currentChatId) {
                    ...
                    // Push the customer's message into the Assist panel
                    // so the agent gets a suggested answer in real time.
...
// Polls every 4 seconds to catch updates when WS broadcasts
// don't arrive (e.g. no Apache proxy configured for /ws)
...
/* ============================================================
   ...
   Talks to the embedded iframe (https://bpoc.in/TataPlayAssist/)
   over postMessage in both directions:

     dashboard -> assist : { type:'agent_assist_query', query }
        sent whenever a NEW customer message arrives in the
        currently open chat, so Assist can auto-search for it

     assist -> dashboard : { type:'agent_assist_answer', answer }
        sent when the agent clicks "Send to Chat" inside the
        Assist panel; we drop the text into #messageBox for the
        agent to review/edit before actually sending it on
============================================================ */
...
function notifyAssist(query) {
    ...
    if (!assistReady) {
        // Iframe hasn't signalled readiness yet - queue the latest
        // query and it'll fire as soon as 'agent_assist_ready' arrives.
